...
export function Navbar({ sidebarExpanded, onToggleSidebar }: NavbarProps) {
  ...
  return (
    <header className={`h-16 fixed right-0 top-0 z-20 bg-background border-b border-border transition-all duration-300 ${sidebarExpanded && !isMobile ? 'left-64' : 'left-0 md:left-16'} `}>
      <div className="flex items-center justify-between h-full px-4 md:px-10">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-base font-medium truncate text-foreground" title={breadcrumb}>{breadcrumb}</span>
        </div>

        {/* Right row: search, theme toggle, add account */}
